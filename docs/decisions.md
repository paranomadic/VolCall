...
| **Admin / testing** | Not in PRD — operator tools. | **`ADMIN_EMAILS`** or **`User.isAdmin`** → **`/admin`**: **App flow tests** (call, payment, Deribit DVOL read APIs), **DVOL thresholds**, **grant comp**. Routes: `POST /api/admin/trigger-call`, `POST /api/admin/payment-test`, `POST /api/admin/dvol-read`, `app/api/admin/*`. Shared checkout: `lib/hosted-checkout.ts`. | Audit log; stricter RBAC. |
...
| Admin | `app/admin`, `app/api/admin/*` (`settings`, `grant-free`, `trigger-call`, `payment-test`, `dvol-read`) |
